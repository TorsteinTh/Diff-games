...
export default class Wiki extends Component {
  ...
  renderText = search_word => {
    const URL =
      "https://en.wikipedia.org/w/api.php?action=query&origin=*&list=search&prop=revisions&rvprop=content&utf8=&format=json";
    const words = search_word.split("\n");
    const search_word_with = "";
    const new_search_words = words.map(e => e + "%20");
    ...
    return (
      <div style={{ alignContent: "center" }}>
        ...
        <form onSubmit={this.handleSubmit}>
          <input onChange={this.handleChange} value={this.state.wantedContent} placeholder="Doesn't work" autoFocus />
        </form>
      </div>
    );
  }
